...
class CATE(nn.Module):
    def __init__(self, input_dim=512, ib_dim=512, interv_dim=512, concept_path="./prompt_feats/brca_concepts.pt",n_classes=2):
        ...
        self.total_concepts = sum([self.feats[i].shape[0] for i in range(self.n_classes)])
        self.interv_linear = nn.Sequential(
            nn.Linear(self.total_concepts, interv_dim),
            nn.ELU(),
    ...
    def ib(self, x, label=None, results_dict=None):
        ...
        if label is not None:
            if label[0] == 0:
                ...
                sim = x @ self.feats[2].mean(dim=0)
                h_pos_2 = x_re[torch.topk(sim, 10)[1]]
                info_loss = self.infonce_loss(h_pos_2, repeat(self.feats[2].mean(dim=0), 'c -> b c', b=h_pos_2.shape[0]), torch.concat([torch.mean(self.feats[j], dim=0, keepdim=True) for j in range(len(self.feats)) if j != 2], dim=0))    # best
            results_dict[f'infonce_loss'] = info_loss

        x_c = (x_re @ torch.concat(self.feats[:self.n_classes], dim=0).T * 56.3477).detach()
